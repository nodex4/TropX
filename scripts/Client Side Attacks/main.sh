...
selectOptions "Client Side Attacks" "Selection" "Try another selection" "Gaining Access" "Monitoring"
if [[ "$SO" == "1" ]]; then   # Gaining Access
  selectOptions "Gaining Acces" "Selection" "Try another selection" "Create Backdoor" "Create Trojan"
  if [[ "$SO" == "1" ]]; then   # Create Backdoor
    installPackages veil
  :
  selectOptions "Backdoor Creation Type" "Selection" "Try another selection" "Express" "Custom"
  if [[ $SO == "1" ]]; then # express
    selectOptions "Backdoor Type" "Selection" "Try another selection" "Windows" "MacOS" "IOS" "Android" "Linux"
    if [[ $SO == "1" ]]; then # Windows
  ...
  elif [[ "$SO" == "2" ]]; then # Create Trojan
    :
  fi

elif [[ "$SO" == "2" ]]; then # Monitoring
  :
fi
